<template>
  <div>
    <div class="banner-pic">
      <img class="ban-img" :src="column_info.cover">
    </div>
    <div class="child-mian" :class="{nobuy:isBuy==0}">
      <div class="column-top white-bg">
        <div style="padding: 0.15rem">
          <div class="title-box clearfix">
            <span class="mark">专栏</span>
            <strong class="title">{{column_info.title}}</strong>
          </div>
          <div class="price free" v-if="column_info.money_type==1">免费</div>
          <div class="price" v-if="column_info.money_type==2 && column_info.limit_time_open==2">¥{{$commonModel.returnFloat(column_info.money)}}</div>
          <div class="learning-info clearfix">
            <div class="fl">
              <span>{{column_info.updated_lectures_desc}}</span>
              <span> | </span>
              <span>{{column_info.study_count}}人在学习</span>
            </div>
            <div class="fr">
              <!--money_type==2收费程课，购买送人-->
              <span v-if="column_info.money_type==2" class="btn54x22" @click="songren()">送人</span>
               <!--money_type==1免费程课，分享地址-->
              <span v-if="column_info.money_type==1" class="btn54x22" @click="column_share()">送人</span>
              <!--<span class="btn54x22" @click="column_share()">分享</span>-->
            </div>
          </div>
        </div>
        <!--限时特价-->
        <div class="limit-money-box" v-if="column_info.money_type==2 && column_info.limit_time_open==1 && limitday">
          <div class="price-box">
            <dl>
              <!--<dt><span class="fs18">¥</span><span>{{$commonModel.returnFloat(column_info.limit_time_money)}}</span></dt>-->
              <dt v-if='zhengshu'>
              ...
              <dd>
                <div class="old-money">原价 ¥{{$commonModel.returnFloat(column_info.money)}}</div>
                <div class="tips">限时活动价格</div>
              </dd>
            </dl>
          </div>
          <!--倒计时-->
          <div v-if="column_info.limit_time_open==1">
      ...
      <div class="column-content white-bg">
        <ul class="child-tab">
          ...
          <li :class="{active:tab_cur==2}" @click="swich_tab(2)"><span>目录</span></li>
          <li :class="{active:tab_cur==3}" @click="swich_tab(3)"><span>评价</span></li>
        </ul>
        <!--详情-->
        <div v-show="tab_cur==1">
          <div v-show="isDetailsHtml==''" class="nodata">暂无内容</div>
        ...
        <div v-show="tab_cur==2">
          ...
          <div class="list-section listItem" v-if="listdata.length>0">
            <ul style="padding-bottom: 0.01rem">
              <li class="flex gametime" @click="course_link(list)" v-for="(list,index) of listdata" :key="index">
                <div class="flex_2 kpic">
                  <img :src="list.cover">
                  <span class="liveicon" v-if="list.course_type==4">直播</span>
                ...
                <div class="flex_6 gamedetails">
                  ...
                  <!--<div class="itembtn itembtnp0">{{list.expected_lectures_desc}}</div>-->
                  <div class="price">
                    <span class="fl audio-length" v-if="list.audio_length&&list.audio_length!=0">{{list.audio_length}}分钟</span>
                    <span class="fr livebtn shiting" style="margin-left: 0.06rem" v-if="column_money_type==2&&list.money_type==1&&list.course_type!=4">试听课</span>
                    <!--  "live_status": 直播状态: 1等待直播 2正在直播 3转码中 4观看回放-->
                    <div class="fr" v-if="list.course_type==4">
                      <span class="fr livebtn load" v-if="list.live_status==1">等待直播</span>
                      <span class="fr livebtn live" v-if="list.live_status==2">正在直播</span>
                      <span class="fr livebtn transcoding" v-if="list.live_status==3">转码中</span>
                      <span class="fr livebtn playback" v-if="list.live_status==4">观看回放</span>
                    </div>
                  </div>
                </div>
              </li>
...
//        endTimes:'1545146603000',
    ...
    mounted(){
      this.init();
      this.column_share("init");//分享默认获取图片
      this.getcomment()
    },
    computed:{
      tab_cur(){
        return this.$store.state.columnMod.tab_cur;
      },
      endTime(){
    ...
    methods: {
      init: function () {
        let that = this;
        let ajaxURL = '/h5/column_info/' + this.$route.query.queryid;
        this.$axios.post(ajaxURL, {
          ...
          if (res.code == 0) {
            ...
            that.column_money_type = data.column_info.money_type;
            if (data.column_info.details) {
              that.isDetailsHtml = data.column_info.details;
            } else {
              that.isDetailsHtml = '';
            }

            var value = Math.round(parseFloat(data.column_info.limit_time_money)) / 100;
            if (value % 1 !== 0) {
              that.zhengshu = false;
              that.xiaoshu = true;
            ...
            that.endTimes = data.column_info.limit_time_end;
            let endTime = new Date(data.column_info.limit_time_end)
      ...
      swich_tab(num){
        this.$store.dispatch('changeColumnTab', num);
//          if(num==3){
//              this.getcomment();
//          }
      },
      course_link(list){
        if (this.isBuy == 1) {
          this.$router.push({path: '/m/course', query: {queryid: list.id}})
        } else {
          this.toggleModal("请您先完成报名！")
        }
      ...
      column_share(init){
        ...
        let title = this.column_info.title;
        let icon = this.column_info.cover;
      ...
      isCollFun(){// 是否收藏
        if (this.is_collect == 0) {
          ...
          this.$axios.post(ajaxURL, {
            ...
            goods_id: that.column_info.id,//标识id
          }).then(res => {
            res = res.data;
            if (res.code == 0) {
              this.is_collect = 1
      ...
      baoming(){//立即报名
        ...
        if(this.is_buy==1){ //是否购买 0 无 1已购买
          ...
          this.to_column_pay(0);
        }


      },
      songren(){//送人
        this.to_column_pay(1)
      },
      to_column_pay(num){
        let orderData = {
...
//          goods_type:1, //类型 默认0 0课程 1专栏
        ...
        this.$router.push({path: '/m/column_pay', query: orderData})
      },
      ...
      getcomment(){
        ...
        this.$axios.post(ajaxURL, {
          column_id: this.$route.query.queryid,//专栏id
