<template>
  <div>
    <div class="head-menu">
      ...
      <scroller :class="{isNotData:nodata}" :on-refresh="refresh" :on-infinite="infinite" ref="my_scroller" style="padding-top:0.44rem;padding-bottom:0.5rem;">
      <div>
        <banner-swiper :list="banner_list" v-if="bannershow"></banner-swiper>
        <div class="child-index-mian" :class="{all:allcolumn==category_id}">
                    ...
                    <div class="flex_6 gamedetails" v-if="list.type==0 || list.type==1"> <!-- 课程 专栏 -->
                        ...
                        <div class="price">
                            ...
                            <span class="fr money" v-if="list.money_type==2 && list.limit_time_open==1 && list.limit_time_money>0">
                              <del class="oldmoney mr10">¥{{$commonModel.returnFloat(list.money)}}</del>¥{{$commonModel.returnFloat(list.limit_time_money)}}
                            </span>
...
//      this.tabs=this.$store.state.schoolMod.navlist;
    ...
    methods: {
      ...
      categoryList:function () {
        ...
        this.$axios.post(ajaxURL, {
          ...
          if (res.data.code == 0) {
            ...
            that.allcolumn=data.category_list[0].id;
      ...
      getlist(done){
        let that= this;
        let ajaxURL='/h5/category_baby_list'
        this.$axios.post(ajaxURL, {
